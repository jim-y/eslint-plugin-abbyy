...
module.exports = {
    ...
    create: function (context) {
        ...
        const isCustomHook = fileName.startsWith('use');
        ...
        function isHookExport(node) {
            ...
            const isFunctionExpression =
                jspath.apply(
                    '.declarations{.id{.type === "Identifier" && .name ^= "use"} && .init{.type === "ArrowFunctionExpression"}}',
                    declaration
                ).length > 0;
        ...
        function countHookSpecifiers(node) {
            const hookAlikeSpecifiers = jspath.apply(
                '.local{.type === "Identifier" && .name ^= "use"}',
                node.specifiers
            );
                    ...
                    return variables.find((variable) => {
                        ...
                        if (variableNode.init) {
                            isFunction =
                                ...
                                variableNode.init.type === 'FunctionExpression';
                        ...
                        return (
                            variable.name === hookAlikeSpecifier.name &&
                            isFunction
                        );
                    });
                });
